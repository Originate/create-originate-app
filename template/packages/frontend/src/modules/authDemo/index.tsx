...
import { useDispatch, useStore } from "../../components/contexts"
import { isBad, isGood, isLoading } from "../../lib"
...
const AuthDemoView: React.FC<{
  ...
  <>
    <p>
      <code>create-originate-app</code> comes with a simple stateless
      authentication system that persists to Postgres. For this to work, you
      will need to run <code>yarn migrate up 100</code> first.
    </p>
    <details>
...
export const AuthDemo: React.FC<{}> = () => {
  ...
  return (
    <AuthDemoView
      ...
      onSignUp={async e => {
        e.preventDefault()
        const [email, password, confirm] = (Array.from(
          e.currentTarget.elements,
        ) as unknown) as HTMLInputElement[]
        await dispatch.authDemo.onSignUp(
          email.value,
          password.value,
          confirm.value,
        )
